...
func createGetServer(t *testing.T) *httptest.Server {
	...
	ts := createTestServer(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == "GET" {
			...
			case "/default-retry-after-second":
				w.Header().Add("Retry-After", "1")
				w.WriteHeader(http.StatusTooManyRequests)
				_, _ = w.Write([]byte(`{ "message": "too many" }`))
			case "/default-retry-after-date":
				untilTime := time.Now().Add(time.Second).UTC().Format(http.TimeFormat)
				w.Header().Add("Retry-After", untilTime)
				w.WriteHeader(http.StatusTooManyRequests)
				_, _ = w.Write([]byte(`{ "message": "too many" }`))
			}
