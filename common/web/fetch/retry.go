...
// return (0, nil) fallback to use jitterBackoff
func defaultRetryAfter(clientInterface ClientInterface, response *Response) (time.Duration, error) {
	if response != nil && response.Header() != nil {
		v, ok := response.Header()["Retry-After"]
		if !ok || len(v) != 1 {
			return 0, nil
		}
		if waitTime, err := strconv.Atoi(v[0]); err == nil {
			return time.Duration(waitTime) * time.Second, nil
		} else if untilTime, err := time.Parse(http.TimeFormat, v[0]); err == nil {
			gmtTimeLoc := time.FixedZone("GMT", 0)
			now := time.Now().In(gmtTimeLoc)
			wait := untilTime.Sub(now).Nanoseconds()
			if wait < 0 {
				wait = 0
			}
