...
func Test_ClientRetryAfter(t *testing.T) {
	ts := createGetServer(t)
	defer ts.Close()

	c := NewRetryableClient(NewClient(hclog.L())).
		WithRetryCount(1)
	c.client.httpClient.Timeout = 2 * time.Second

	start := time.Now()
	resp, _ := c.Get(ts.URL + "/default-retry-after-second").Execute()
	assert.Equal(t, http.StatusTooManyRequests, resp.StatusCode())
	assert.Equal(t, "1", resp.Header()["Retry-After"][0])
	// retry Wait 1s, total used time should be about 1s
	end := time.Now()
	assert.True(t, end.Sub(start).Milliseconds() > 1000)
	assert.True(t, end.Sub(start).Milliseconds()-int64(1000) < 5)

	start = time.Now()
	resp, _ = c.Get(ts.URL + "/default-retry-after-date").Execute()
	assert.Equal(t, http.StatusTooManyRequests, resp.StatusCode())
	// retry Wait 1s, total used time should be about 1s
	end = time.Now()
	assert.True(t, end.Sub(start).Milliseconds()-int64(1000) < 5)
