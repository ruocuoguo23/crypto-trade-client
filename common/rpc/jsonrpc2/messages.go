...
func EncodeBatchMessage(batchMsg []Message) ([]byte, error) {
	var batchWire []wireCombined
	for _, msg := range batchMsg {
		wire := wireCombined{VersionTag: wireVersion}
		msg.marshal(&wire)

		batchWire = append(batchWire, wire)
	}

	data, err := json.Marshal(&batchWire)
	if err != nil {
		return data, fmt.Errorf("marshaling jsonrpc batch message: %w", err)
	}
...
func DecodeMessage(data []byte) (Message, error) {
	...
	message, err := wireCombinedToMessage(msg)
	if err != nil {
		return nil, err
	}
...
func wireCombinedToMessage(wire wireCombined) (Message, error) {
	if wire.VersionTag != wireVersion {
		return nil, fmt.Errorf("invalid message version tag %s expected %s", wire.VersionTag, wireVersion)
	}
	id := ID{}
...
func DecodeBatchMessage(data []byte) ([]Message, error) {
	var batchWire []wireCombined
	if err := json.Unmarshal(data, &batchWire); err != nil {
		return nil, fmt.Errorf("unmarshaling jsonrpc batch message: %w", err)
	}

	var batchMessage []Message
	for _, wire := range batchWire {
		message, err := wireCombinedToMessage(wire)
		if err != nil {
			return nil, err
		}

		batchMessage = append(batchMessage, message)
	}
...
// Responses is the type of []*Response
